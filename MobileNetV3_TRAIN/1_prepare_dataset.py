"""Подготовка тренировочного и валидационного наборов из distrib.
...
должен собираться заново из ошибок НОВОЙ модели). Очистка удаляет все
RETRAIN_DIR/{0,90,180,270} и review_report.csv. Отключить: флаг --keep-retrain.
"""
import argparse
import random
import shutil
...
def main():
    parser = argparse.ArgumentParser(description="Подготовка train/val из distrib.")
    ...
    print(f"Готово. train={total_train} val={total_val}")
