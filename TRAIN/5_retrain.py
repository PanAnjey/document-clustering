...
- Опционально заменяет лучшую модель новой версией.
...
def main():
    ...
    # Пробуем загрузить как полный объект модели (новый формат)
    try:
        model = torch.load(base_model_path, map_location=device, weights_only=False)
        print(f"[INFO] Загружена модель (новый формат): {base_model_path.name}")
    except Exception:
        # Fallback: старый формат (state_dict)
        print(f"[INFO] Загрузка модели (старый формат state_dict)...")
        ckpt = torch.load(base_model_path, map_location=device, weights_only=True)
        model = build_model(C.OUTPUT_CLASSES, device)
        model.load_state_dict(ckpt["model_state"], strict=False)
        print(f"[INFO] Веса загружены из: {base_model_path.name}")
    ...
    # Fine-tuning: замораживаем backbone, обучаем только head
    ...
    best_score = -1.0
    # Подсчёт существующих retrain моделей
    existing_retrain = list(C.MODELS_DIR.glob(f"{C.RETRAIN_FILE_PREFIX}_*.pth"))
    retrain_path = C.MODELS_DIR / f"{C.RETRAIN_FILE_PREFIX}_{len(existing_retrain) + 1}.pth"
    ...
    for epoch in range(1, C.RETRAIN_EPOCHS + 1):
        ...
        if score > best_score:
            best_score = score
            torch.save(model, retrain_path)  # сохраняем всю модель целиком
