...
OLD_FT_OVERALL = 0.903          # train2_full, случайный сплит с утечкой шаблонов
OLD_ZS_4B = 0.692              # train2_full, та же база вхолодную, тот же протокол
...
def main():
    ...
    L.append(f'Прирост от дообучения: **+{(ftm - zsm) * 100:.1f} п.п.** '
             f'({zsm:.1%} → {ftm:.1%}).')
    L.append(f'Старый замер (случайный сплит с утечкой шаблонов): '
             f'дообученная {OLD_FT_OVERALL:.1%}, вхолодную {OLD_ZS_4B:.1%}.')
