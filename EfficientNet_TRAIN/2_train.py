"""Дообучение EfficientNet-B1 для классификации ориентации сканов (4 класса: 0,90,180,270).

- Загружает torchvision efficientnet_b1 с предобученными весами IMAGENET1K_V2.
...
class OrientationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 2) Вычисляем новый угол после поворота (всегда кратен 90)
        new_angle = (orig_angle + a) % 360
        target = self.angle_to_idx.get(new_angle, 0)  # fallback на 0° если класс отсутствует

        # 3) VerticalFlip для улучшения 0° vs 180° (только train)
        if self.mode == "train" and getattr(C, "VERTICAL_FLIP_AUG", False) and random.random() < getattr(C, "VERTICAL_FLIP_PROB", 0.5):
            img = img.transpose(Image.FLIP_TOP_BOTTOM)
            
            # Меняем метки при вертикальном флипе: 0° ↔ 180°, 90° ↔ 270°
...
def build_model(num_classes):
    """Создать EfficientNet-B1 с заменой классификатора под num_classes."""
